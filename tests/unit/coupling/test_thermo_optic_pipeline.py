"""The thermo-optic effect end to end: declaration, sampling, perturbation after the blend.
...
def _coupling(dn_core: float = DN_CORE, dn_bg: float = DN_BG) -> ThermoOptic:
    return ThermoOptic(dn_dT={"core": dn_core, "bg": dn_bg}, reference_temperature=T_REF)


def perturbed_permittivity(permittivity, dn_dT, delta_T):
    """``(sqrt(eps) + dn/dT * dT)^2``, elementwise: the index law the response applies.

    The class states it as ``ThermoOptic.perturbed_permittivity(material, index, temperature)``;
    this is the same arithmetic in the (permittivity, coefficient, temperature *change*) form the
    expectations below are written in, so the numbers they pin are unchanged.
    """
    coupling = ThermoOptic(dn_dT={"m": 1.0}, reference_temperature=0.0)
    return coupling.perturbed_permittivity("m", np.sqrt(permittivity), dn_dT * delta_T)
...
def test_a_uniform_temperature_reproduces_the_pre_perturbed_scene_at_every_entry(float64, placement, full_tensor):
    ...
    perturbed, report = _coupling().perturb(arrays, info, materials, samples)
    ...
    assert report.extras["max_delta_n"] == pytest.approx(DN_CORE * dT)
...
def test_a_zero_temperature_change_is_the_identity(float64):
    ...
    perturbed, _ = _coupling().perturb(arrays, info, materials, samples)
...
def test_a_gradient_field_gives_the_closed_form_in_the_bulk_and_local_kottke_at_the_interface(float64):
    ...
    inv_eps, offdiag, report = apply_permittivity_perturbation(
...
def test_an_uncovered_perturbed_point_is_an_error_by_default_and_a_counted_no_op_on_request(float64):
    ...
    with pytest.raises(ValueError, match="outside the sampled field"):
        _coupling().perturb(arrays, info, materials, samples)

    perturbed, report = _coupling().perturb(arrays, info, materials, samples, uncovered="unperturbed")
...
def test_a_material_without_a_coefficient_needs_no_coverage(float64):
    ...
    _, report = ThermoOptic(dn_dT={"core": DN_CORE}, reference_temperature=T_REF).perturb(
        arrays, info, materials, samples
    )
    assert all(v == 0 for v in report.num_uncovered.values())
    assert "bg" not in report.extras["coefficients"]
...
def test_the_nine_component_tier_is_refused(float64):
    ...
    with pytest.raises(NotImplementedError, match="3-component"):
        _coupling().perturb(arrays, info, materials, uniform_samples(config.resolved_grid, T_REF + 1.0))
...
def test_two_materials_of_one_value_class_must_share_a_coefficient(float64):
    ...
    with pytest.raises(ValueError, match="same value"):
        ThermoOptic(dn_dT={"core": 1e-4, "bg": 2e-4}, reference_temperature=T_REF).perturb(
            arrays,
            info,
            materials,
    ...
    _, report = ThermoOptic(dn_dT={"core": 1e-4, "bg": 1e-4}, reference_temperature=T_REF).perturb(
        arrays,
        info,
        materials,
        uniform_samples(config.resolved_grid, T_REF + 1.0),
...
def test_an_unknown_material_name_is_an_error(float64):
    ...
    with pytest.raises(KeyError, match="absent from the scene"):
        ThermoOptic(dn_dT={"silicon": 1e-4}, reference_temperature=T_REF).perturb(
            arrays,
            info,
            materials,
            uniform_samples(config.resolved_grid, T_REF + 1.0),
...
def test_the_box_sampling_mode_has_no_material_map():
    ...
    with pytest.raises(ValueError, match="yee_material_map"):
        ThermoOptic(dn_dT={"bg": 1e-5}, reference_temperature=T_REF).perturb(
            arrays,
            info,
            materials,
            uniform_samples(config.resolved_grid, T_REF + 1.0),
...
def test_yee_lattice_samples_round_trip_through_npz(float64, tmp_path):
    ...
    a, _ = _coupling().perturb(arrays, info, materials, samples, uncovered="unperturbed")
    b, _ = _coupling().perturb(arrays, info, materials, back, uncovered="unperturbed")
