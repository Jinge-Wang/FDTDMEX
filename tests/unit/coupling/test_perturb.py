...
Pins that the ``ThermoOptic`` coupling and the general engine agree bit for bit; that a uniform
...
def test_the_thermo_optic_coupling_and_the_general_engine_agree_bit_for_bit(float64):
    ...
    a_inv, a_off, _ = apply_permittivity_perturbation(
...
def test_an_off_diagonal_bulk_tensor_is_refused_on_the_diagonal_tier(float64):
    ...
    samples = samples_from_callable(
        config.resolved_grid, lambda p: np.tile([5.0e6, 0.0, 0.0], (p.shape[0], 1)), name="E", unit="V/m"
    )
    model = PerturbationModel({"core": PockelsResponse(r=r_3m())})
...
def test_the_physics_validation_catches_a_tensor_that_stops_being_positive_definite(float64):
    ...
    from fdtdx.coupling.responses import MaterialResponse
...
def test_the_photoelastic_response_is_the_identity_at_zero_strain_and_moves_with_strain(float64):
    ...
    zero = samples_from_callable(grid, lambda q: np.zeros((q.shape[0], 6)), name="S", unit="1")
    ...
    S = 1e-3
    strained = samples_from_callable(grid, lambda q: np.tile([S, 0, 0, 0, 0, 0], (q.shape[0], 1)), name="S", unit="1")
...
def test_the_thermo_optic_coupling_still_refuses_an_anisotropic_material(float64):
    ...
    with pytest.raises(NotImplementedError, match="anisotropic"):
        ThermoOptic(dn_dT={"core": 1e-4}, reference_temperature=T_REF).perturb(
            arrays,
            info,
            materials,
