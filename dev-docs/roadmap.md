...
- **2nd-order non-uniform (graded) grids.** Spacing-weighted curl, interpolation, and off-diagonal averaging (2nd-order vs a plain unweighted mean's 1st-order). The off-diagonal averaging fix was originated here and **upstreamed — merged into fdtdx as #378** (now in both). Depth: [nonuniform-grid.md](../docs/nonuniform-grid.md).
