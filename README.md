...
- **2nd-order accurate non-uniform (graded) grids** — spacing-weighted curl, field interpolation, *and* off-diagonal anisotropic averaging, all 2nd-order on graded meshes. The **width-weighted off-diagonal averaging for full-tensor anisotropy** (2nd-order where a plain 4-point mean is only 1st-order on a graded mesh) was **developed in this project and contributed upstream — now merged into fdtdx (#378)** — so it ships in *both* fdtdx and this fork today (the MLX engine mirrors it element-for-element; on uniform / per-axis-uniform grids every weighted form reduces byte-identically to the plain mean). An accuracy improvement this project *originated and upstreamed*, no longer a fork-vs-upstream difference.
- **Mesh override regions** — `GradedGrid` takes a background cell width plus `RefinementRegion` boxes in physical coordinates and generates the rectilinear mesh: cells inside a region are at most its target, widths grade to the background within a neighbour-ratio bound, region boundaries land on cell edges, and the extent is exact. A slab transmission on a 50 nm background with a 12.5 nm region lands 3.9e-4 from the uniformly fine answer using 220 z-cells instead of 320. See [docs/nonuniform-grid.md](docs/nonuniform-grid.md) and [examples/graded_grid_slab/](examples/graded_grid_slab/).
...
- **Analysis utilities** — a **filter-diagonalisation resonance finder** (`find_resonances`): complex frequency, Q, amplitude and phase of every mode in a band, read off a short ring-down rather than a long DFT; plus a Lorentzian line fit for measured spectra, an envelope ring-down Q cross-check, and a caching **parameter-sweep runner** (`run_sweep`) that turns build/evaluate into a CSV-able table. See [docs/analysis-utilities.md](docs/analysis-utilities.md).
...
**Non-uniform (graded) grids — 2nd-order accurate.** FDTDMEX's spacing-weighted operators converge at 2nd order on graded meshes (measured slope ≈ 2.0). The key accuracy piece is the **off-diagonal anisotropic average**: a plain unweighted 4-point mean degrades a full-tensor anisotropic update to 1st order on a graded grid, whereas the width-weighted average holds 2nd order. This project originated that fix and **contributed it upstream — merged into fdtdx as #378** — so upstream and this fork now both hold 2nd order (the MLX engine mirrors it element-for-element).
